...
pio_init.bat
  ...
  │   ├─ uv python install 3.12.12 --no-shim → 下载完整 Python
  │   ├─ 复制到 python/ 目录
...
```
warning: Failed to install executable for cpython-3.12.12
  Caused by: Executable already exists at C:\Users\...\.local\bin\python3.12.exe but is not managed by uv
```
